...
export async function Product({ name, description }: ProductsProps) {
  return (
    <div
      className={`${lusitana} m-3 flex flex-col flex-wrap content-center gap-6 rounded-lg p-3 text-center`}
    >
      <div>{name}</div>
      <div>{description}</div>
      <Link
        href="/"
        className="rounded-lg bg-blue-400 px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-blue-300 md:text-base"
      >
        <span>{`See ${name}s`}</span>
      </Link>
    </div>
    ...
    //     {/* TODO: Get all Products and list them out here */}
